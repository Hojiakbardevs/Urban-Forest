...
import { ChevronLeft, ChevronRight, Layers, Sliders, MapPin, Thermometer, Leaf } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className={`fixed left-0 top-16 h-[calc(100vh-4rem)] w-80 bg-white/95 backdrop-blur-md border-r border-white/20 shadow-xl transition-transform duration-300 z-50 ${
        isOpen ? 'translate-x-0' : '-translate-x-full'
      }`}>
        <div className="h-full flex flex-col">
          <div className="p-6 border-b border-gray-200">
            <h2 className="text-xl font-semibold text-gray-900 mb-2">Map Controls</h2>
            <p className="text-sm text-gray-600">Configure layers and analyze heat patterns</p>
          </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm text-gray-600">Planting Zones</span>
                  </div>
                  <span className="text-sm font-medium">142 areas</span>
                </div>
              </div>
            </div>
          </div>
