...
import { DrawnPolygon, PlantingRecommendation, SeasonalGrowth, PredefinedZone } from '../types/planting';
...
const TreePlantingVisualizer: React.FC = () => {
  ...
  const getTreeSpecies = (area: number): string[] => {
    if (area > 10000) {
      return ['Platanus orientalis', 'Populus nigra', 'Acer platanoides', 'Tilia cordata'];
    } else if (area > 5000) {
  ...
  const generateSeasonalGrowth = (baseNDVI: number, targetNDVI: number): SeasonalGrowth[] => {
    const months = [
    ...
    return months.map((month, index) => {
      ...
      const seasonalMultiplier = Math.sin((index / 12) * Math.PI * 2 + Math.PI / 2) * 0.3 + 0.7;
      const ndviValue = baseNDVI + (targetNDVI - baseNDVI) * progress * seasonalMultiplier;
      const coveragePercent = Math.min(100, progress * 100 * seasonalMultiplier);

      return {
        month: index,
        ndviValue: Math.max(baseNDVI, ndviValue),
        coveragePercent,
        description: `${month}: ${coveragePercent.toFixed(0)}% vegetation coverage`
      };
    });
  };

  // Calculate polygon properties
  const calculatePolygonProperties = useCallback((layer: L.Polygon): DrawnPolygon => {
    ...
    return {
      ...
      area,
      centroid: [centroidFeature.geometry.coordinates[1], centroidFeature.geometry.coordinates[0]],
      boundingBox: {
        north: bbox[3],
        south: bbox[1],
        east: bbox[2],
  ...
  const generateRecommendation = useCallback((polygon: DrawnPolygon): PlantingRecommendation => {
    ...
    const projectedNDVI = Math.min(0.8, currentNDVI + 0.25 + (treeCount / polygon.area) * 1000);
    ...
    if (showPredefinedZones) {
      predefinedZones.forEach(zone => {
        const layer = L.geoJSON(zone.geometry, {
          style: {
            color: zone.priority === 'high' ? '#dc2626' : zone.priority === 'medium' ? '#f59e0b' : '#10b981',
            weight: 2,
            fillOpacity: 0.1,
            dashArray: '5, 5'
          }
        ...
        layer.bindPopup(`
          <div class="p-3">
            ...
            <p><strong>Area:</strong> ${(zone.area / 1000).toFixed(1)} hectares</p>
            <p><strong>Priority:</strong> ${zone.priority}</p>
            <p><strong>Existing Vegetation:</strong> ${(zone.existingVegetation * 100).toFixed(0)}%</p>
          </div>
        `);
  ...
  return (
    <div className="h-screen flex flex-col bg-gradient-to-br from-green-50 to-blue-50">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <div className="flex items-center space-x-2 bg-gradient-to-r from-green-600 to-blue-600 text-white px-4 py-2 rounded-lg">
                ...
                <span className="text-xl font-bold">Tree Planting Visualizer</span>
              </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="text-right">
                ...
                <div className="text-xs text-gray-500">Tree Planting Optimization</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex relative">
        {/* Sidebar */}
        <div className={`${sidebarOpen ? 'w-96' : 'w-0'} transition-all duration-300 overflow-hidden bg-white/95 backdrop-blur-md border-r border-white/20 shadow-xl`}>
          <div className="h-full flex flex-col">
            <div className="p-6 border-b border-gray-200">
              <h2 className="text-xl font-semibold text-gray-900 mb-2">Planting Analysis</h2>
              <p className="text-sm text-gray-600">Draw areas to visualize future vegetation growth</p>
            </div>

            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              {/* Drawing Controls */}
              <DrawingControls
                onTogglePredefinedZones={() => setShowPredefinedZones(!showPredefinedZones)}
              ...
              {selectedPolygon && selectedRecommendation && (
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <h3 className="font-medium text-gray-900">Growth Timeline</h3>
                ...
                <div className="flex items-start space-x-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <ul className="text-sm text-blue-700 space-y-1">
                      <li>• Click the polygon tool in the map to start drawing</li>
                      <li>• Draw around areas where you want to plant trees</li>
                      <li>• View instant calculations and recommendations</li>
                      <li>• Use the timeline to see future growth simulation</li>
                      <li>• Toggle predefined zones for comparison</li>
                    </ul>
                  </div>
                </div>
              </div>
