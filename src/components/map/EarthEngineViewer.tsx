...
import { Flame, Layers, RotateCcw, Info, Thermometer, Leaf } from 'lucide-react';

interface EarthEngineViewerProps {
  className?: string;
}

const EarthEngineViewer: React.FC<EarthEngineViewerProps> = ({ className = '' }) => {
  ...
  useEffect(() => {
    ...
    const handleError = () => {
      setIsLoading(false);
      setError('Unable to load Earth Engine heatmap. Please check your connection.');
    };
      ...
      {isLoading && !error && (
        <div className="absolute inset-0 bg-gray-900 flex items-center justify-center z-20">
          <div className="text-center text-white">
            ...
            <p className="text-lg font-medium">Loading Earth Engine Heatmap...</p>
            <p className="text-sm text-gray-300 mt-2">
              Analyzing urban heat patterns in Tashkent
            </p>
          </div>
        </div>
      )}
      ...
      {error && (
        <div className="absolute inset-0 bg-gray-900 flex items-center justify-center z-20">
          <div className="text-center text-white max-w-md mx-auto p-6">
            ...
            <p className="text-lg font-medium mb-2">Heatmap Unavailable</p>
      ...
      <div className="absolute bottom-4 left-4 z-10 instructions-panel rounded-lg shadow-lg p-4 max-w-sm">
        <div className="text-sm text-gray-700">
          <div className="flex items-center space-x-2 mb-2">
            ...
            <span className="font-medium text-gray-900">Heatmap Features:</span>
          </div>
          <ul className="space-y-1 text-xs">
            <li>• <strong>Pan:</strong> Click and drag to move around</li>
            <li>• <strong>Zoom:</strong> Use mouse wheel or zoom controls</li>
            <li>• <strong>Layers:</strong> Toggle different data layers</li>
            <li>• <strong>Click:</strong> Get temperature readings</li>
          </ul>
          <div className="mt-2 pt-2 border-t border-gray-200">
            <div className="flex items-center space-x-1">
              <Leaf className="w-3 h-3 text-green-600" />
              <span className="text-xs text-green-600">
                Identify hotspots for tree planting priority
              </span>
            </div>
          </div>
        </div>
